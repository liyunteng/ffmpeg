...
int test()
{
    ...
    fp_open = fopen("/Users/lyt/abc.ts", "rb");
    fp_write = fopen("./abc.h264", "wb+");
    ...
    for (i = 0; i < 1; i++) {
        ...
        if (dec_ctx->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            av_log(NULL, AV_LOG_FATAL, "Elementray stream #%d is of unknown type\n", i);
    ...
    if (ret < 0) {
        av_log(NULL, AV_LOG_ERROR, "Erorr occurred when opeing output file\n");
        return ret;
    }
...
/* Local Variables: */
/* compile-command: "clang -Wall -o av_mem av_mem.c -g -lavformat -lavcodec -lavutil" */
/* End: */
