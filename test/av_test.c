...
void custom_output(void *ptr, int level, const char *fmt, va_list va) {
#if 0
    FILE *fp = fopen("av_log.log", "a+");
    if (fp) {
        vfprintf(fp, fmt, va);
        fflush(fp);
        fclose(fp);
...
void print_opt(const AVOption *opt) {
  printf("==============================\n");
  ...
  case AV_OPT_TYPE_RATIONAL:
    printf("[type]rational\n[default]%d/%d\n", opt->default_val.q.num,
           opt->default_val.q.den);
    break;
  default:
    printf("[type]others\n");
    break;
...
void list_obj_opt(void *obj) {
  ...
  printf("==============================\n");
  printf("Video param:\n");
  av_opt_show2(obj, stdout, AV_OPT_FLAG_VIDEO_PARAM, NULL);
  printf("Audio param:\n");
  av_opt_show2(obj, stdout, AV_OPT_FLAG_AUDIO_PARAM, NULL);
  printf("Decoding param:\n");
  av_opt_show2(obj, stdout, AV_OPT_FLAG_DECODING_PARAM, NULL);
  printf("Encoding param:\n");
  av_opt_show2(obj, stdout, AV_OPT_FLAG_ENCODING_PARAM, NULL);
  printf("==============================\n");
}
...
int flush_encoder(AVFormatContext *fmt_ctx, unsigned int stream_index) {
  ...
  while (1) {
    ...
    printf("Success to encode 1 frame!\n");
  }
...
int encoder() {
  ...
  FILE *in_file = fopen("/Users/lyt/abc.yuv", "rb");
  if (!in_file) {
      printf("fopen failed\n");
      return -1;
  }
...
#ifdef TEST_OPT
  av_opt_set(pCodecCtx, "b", "400000", 0);
  av_opt_set(pCodecCtx, "time_base", "1/25", 0);
  av_opt_set(pCodecCtx, "bf", "5", 0); /* max b frame */
  av_opt_set(pCodecCtx, "g", "25", 0); /* gop */
  av_opt_set(pCodecCtx, "qmin", "10", 0);
...
void test_dict() {
  ...
  av_dict_set(&d, "name", "liyunteng", 0);
  av_dict_set(&d, "email", "li_yunteng@163.com", 0);
  av_dict_set(&d, "school", "jlu", 0);
...
void test_log() {
  ...
  obj = avformat_alloc_context();
  printf("==============================\n");
  av_log(obj, AV_LOG_PANIC, "Panic\n");
  av_log(obj, AV_LOG_FATAL, "Fatal\n");
  av_log(obj, AV_LOG_ERROR, "Error\n");
  av_log(obj, AV_LOG_WARNING, "Warning\n");
  av_log(obj, AV_LOG_INFO, "Info\n");
  av_log(obj, AV_LOG_VERBOSE, "Verbose\n");
  av_log(obj, AV_LOG_DEBUG, "Debug\n");
  printf("==============================\n");
  avformat_free_context(obj);
}
...
/* Local Variables: */
/* compile-command: "clang -Wall -o av_test av_test.c -g -lavutil -lavcodec -lavformat" */
/* End: */
