...
const static char *mpeg4audio_profile[]    = {"Main", "LC", "SSR", "LTP"};
...
void
skip_bits(bits_context_t *s, int n)
{
    s->index += n;
}

static inline uint32_t
get_bits(bits_context_t *s, int n)
{
    register int tmp;
    unsigned int re_index = s->index;
    int re_cache          = 0;

    re_cache = AV_RB32(((const uint8_t *)s->buffer) + (re_index >> 3))
               << (re_index & 0x07);
...
ff_aac_parse_header(bits_context_t *gbc, AACADTSHeaderInfo *hdr)
{
    ...
    hdr->number_of_raw_data_blocks_in_frame =
        get_bits(gbc, 2); /* number_of_raw_data_blocks_in_frame */
...
int
getADTSframe(unsigned char *buffer, int buf_size, unsigned char *data,
             int *data_size)
{
    int size = 0;
    ...
    while (1) {
        if (buf_size < AAC_ADTS_HEADER_SIZE) {
            return -1;
        }
...
aac_parse(char *url)
{
    ...
    input_data = (uint8_t *)malloc(stat.st_size);
    if (!input_data) {
        printf("malloc failed\n");
        return -1;
    }

    data_size = fread(input_data, 1, stat.st_size, ifile);
    if (data_size != stat.st_size) {
        printf("fread failed\n");
        return -1;
    }
    ...
    printf("--------+------------------------- AAC Table "
           "-------------------------+--------+\n");
    printf(" Number | Profile | Frequency | Crc | Ch | Samples | Bitrate | "
           "Frames | Length |\n");
    printf("--------+---------+-----------+-----+----+---------+---------+-----"
           "---+--------+\n");
    while (data_size > 0) {
        rc = ff_aac_parse_header(&s, &info);
        if (rc < 0) {
            /* parse failed */
            printf("Parse Failed\n");
            break;
        }
        printf(" %6d | %7s | %7dHz | %3d | %2d | %7d | %7d | %6d | %6d |\n",
               cnt, mpeg4audio_profile[info.profile], info.sample_rate,
               info.crc_absent, info.channel_configuration, info.samples,
               info.bit_rate, info.num_aac_frames, info.frame_length);
        cnt++;
        parsed_size += info.frame_length;
        data_size -= info.frame_length;
        get_bits(&s, (info.frame_length - AAC_ADTS_HEADER_SIZE) * 8);
    }
    printf("--------+---------+-----------+-----+----+---------+---------+-----"
           "---+--------+\n");
    printf("File: %s\nLength: %ld\nParsed: %lu\n", url, stat.st_size, parsed_size);
    fclose(ifile);
    return 0;
}
